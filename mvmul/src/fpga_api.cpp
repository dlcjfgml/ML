...
FPGA::FPGA(off_t data_addr, off_t output_addr, int m_size, int v_size)
{
	...
	output_ = new unsigned int[m_size_];    // use output_ as tempolary output
...
void FPGA::largeMV(const float* large_mat, const float* input, float* output, int num_input, int num_output)
{
	...
	for(int i = 0; i < num_output; i += m_size_)
	{
		for(int j = 0; j < num_input; j += v_size_)
		{
			...
			int block_row = min(m_size_, num_output-i); //행 가로
			int block_col = min(v_size_, num_input-j);
			// !) Assign a vector
			/* IMPLEMENT */
			memset(vec, 0, sizeof(float)*v_size);
			memcpy(vec, input + j, sizeof(float)*block_col);
			// 2) Assign a matrix
			/* IMPLEMENT */
			memset(mat, 0, sizeof(float)*(m_size_ * v_size_));
            for(int a=0; a< block_row; a++ ) {
                memcpy(mat+ a*v_size_, large_mat + j + (a+i)*num_input, sizeof(float)*block_col);
            }
